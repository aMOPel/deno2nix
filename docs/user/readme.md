...
- *`denoDepsHash`* (String): The output hash of the `buildDenoDeps` fixed output derivation.
...
- *`denoDepsImpureEnvVars`* (Array of strings; optional): Names of impure environment variables passed to the `buildDenoDeps` derivation. They are forwarded to `deno install`.
...
- *`denoDepsInjectedEnvVars`* (Attrset; optional): Environment variables as key value pairs. They are forwarded to `deno install`.
...
**WARNING**: Hardcoding a token into your NixOS configuration or some other nix build, will as a consequence write that token into `/nix/store`, which is considered world readable.
...
Either via passthru on `buildDenoPackage`:
...
Or by directly using `fetchDenoDeps`:
